...
export const PRFQVendorsPopup = ({ isOpen, onClose, prfqId, prfqCode }: Props) => {
  ...
  const { data, isLoading } = usePRFQVendors(isOpen ? prfqId : undefined);
      ...
      <ModalContent maxWidth="80vw">
        <ModalHeader textAlign="center">
          <Text fontSize="lg" fontWeight="bold">
            {prfqCode ? `${prfqCode}` : 'PRFQ'} — Vendors
          </Text>
        </ModalHeader>
        <ModalCloseButton />
          ...
          <LoadingOverlay isLoading={isLoading} style={{ minHeight: '20vh' }}>
            ...
            <HStack justify="space-between" mb={3}>
              ...
              <Button
                colorScheme="brand"
                size="sm"
                isDisabled={selectedIds.length === 0 || selectedIds.length > 0}
            ...
            <TableContainer border="1px" borderColor="gray.500" boxShadow="md">
              <Table variant="striped" size="sm">
                ...
                <Tbody>
                  {vendors.length > 0
                    ? vendors.map((vendor: PRFQVendorInfo, index: number) => {
