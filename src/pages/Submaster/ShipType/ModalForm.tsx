...
import { useCreateShipTypes, useUpdateShipTypes } from '@/services/submaster/ship-types/services';
...
const ModalForm = ({
  ...
  const createItem = useCreateShipTypes({
    onSuccess: () => {
      onClose();
      toastSuccess({
        title: 'Ship Type created',
      });
      queryClient.invalidateQueries(['shipTypesIndex']);
    },
    onError: (error) => {
      toastError({
        title: 'Error creating Ship Type',
        description: error.response?.data.message,
      });
    },
  });
  
  const updateItem = useUpdateShipTypes({
    onSuccess: () => {
      onClose();
      toastSuccess({
        title: 'Ship Type updated',
      });
      queryClient.invalidateQueries(['shipTypesIndex']);
    },
    onError: (error) => {
      toastError({
        title: 'Error updating Ship Type',
        description: error.response?.data.message,
      });
    },
  });
    ...
    <Modal initialFocusRef={initialRef} isOpen={isOpen} onClose={onClose} closeOnOverlayClick={false}  closeOnEsc={false}>
      ...
      <ModalContent>
        <Formiz autoForm connect={form}>
          <ModalHeader>{isEdit ? 'Update ' : 'Add '} Ship Type</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
             <FieldInput 
              name="name" 
              placeholder="Enter Ship Type" 
              required={'Ship Type required'}
              type="all-capital"
