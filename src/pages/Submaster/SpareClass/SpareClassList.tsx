...
const SpareClassList = () => {
  ...
  const {
    ...
    refetch: refreshData,
  } = useSpareClassIndex(queryParams);
  ...
  const handleSoftDelete = (item: DataColumn, reason: string) => {
    ...
    handleDelete({
      ...
      permanent: false,
      url: endPoints.soft_delete.spare_class.replace(':id', String(item.id)),
    });
  };
  ...
  const handleRestore = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.restore.spare_class.replace(':id', String(item.id)),
    });
  };
  ...
  const handlePermanentDelete = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.delete.spare_class.replace(':id', String(item.id)),
    });
  };
