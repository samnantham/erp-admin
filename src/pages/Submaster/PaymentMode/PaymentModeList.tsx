...
const PaymentModeList = () => {
  ...
  const {
    ...
    refetch: refreshData,
  } = usePaymentModeIndex(queryParams);
  ...
  const handleSoftDelete = (item: DataColumn, reason: string) => {
    ...
    handleDelete({
      ...
      permanent: false,
      url: endPoints.soft_delete.payment_mode.replace(':id', String(item.id)),
    });
  };
  ...
  const handleRestore = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.restore.payment_mode.replace(':id', String(item.id)),
    });
  };
  ...
  const handlePermanentDelete = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.delete.payment_mode.replace(':id', String(item.id)),
    });
  };
