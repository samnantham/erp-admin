...
import { useCreateShipModes, useUpdateShipModes } from '@/services/submaster/ship-modes/services';
...
const ModalForm = ({
  ...
  const createItem = useCreateShipModes({
    onSuccess: () => {
      onClose();
      toastSuccess({
        title: 'Ship Mode created',
      });
      queryClient.invalidateQueries(['shipModesIndex']);
    },
    onError: (error) => {
      toastError({
        title: 'Error creating Ship Mode',
        description: error.response?.data.message,
      });
    },
  });
  
  const updateItem = useUpdateShipModes({
    onSuccess: () => {
      onClose();
      toastSuccess({
        title: 'Ship Mode updated',
      });
      queryClient.invalidateQueries(['shipModesIndex']);
    },
    onError: (error) => {
      toastError({
        title: 'Error updating Ship Mode',
        description: error.response?.data.message,
      });
    },
  });
    ...
    <Modal initialFocusRef={initialRef} isOpen={isOpen} onClose={onClose} closeOnOverlayClick={false}  closeOnEsc={false}>
      ...
      <ModalContent>
        <Formiz autoForm connect={form}>
          <ModalHeader>{isEdit ? 'Update ' : 'Add '} Ship Mode</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
            <FieldInput 
              name="name" 
              placeholder="Enter Ship Mode" 
              required={'Ship Mode required'}
              type="all-capital"
