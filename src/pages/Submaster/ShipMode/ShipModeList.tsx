...
const ShipModesList = () => {
  ...
  const closeModal = () => {
    queryClient.invalidateQueries(['shipModeIndex']);
  ...
  const handleSoftDelete = (item: DataColumn, reason: string) => {
    ...
    handleDelete({
      ...
      permanent: false,
      url: endPoints.soft_delete.ship_mode.replace(':id', String(item.id)),
    });
  };
  ...
  const handleRestore = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.restore.ship_mode.replace(':id', String(item.id)),
    });
  };
  ...
  const handlePermanentDelete = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.delete.ship_mode.replace(':id', String(item.id)),
    });
  };
