...
import { useCreateCustomEntry, useUpdateCustomEntry } from '@/services/submaster/customentry/services';
...
const ModalForm = ({
  ...
  const createItem = useCreateCustomEntry({
    onSuccess: () => {
      onClose();
      toastSuccess({
        title: 'Custom Entry created',
      });
      queryClient.invalidateQueries(['customEntryIndex']);
    },
    onError: (error) => {
      toastError({
        title: 'Error creating Custom Entry',
        description: error.response?.data.message,
      });
    },
  });
  
  const updateItem = useUpdateCustomEntry({
    onSuccess: () => {
      onClose();
      toastSuccess({
        title: 'Custom Entry updated',
      });
      queryClient.invalidateQueries(['customEntryIndex']);
    },
    onError: (error) => {
      toastError({
        title: 'Error updating Custom Entry',
        description: error.response?.data.message,
      });
    },
  });
    ...
    <Modal initialFocusRef={initialRef} isOpen={isOpen} onClose={onClose} closeOnOverlayClick={false}  closeOnEsc={false}>
      ...
      <ModalContent>
        <Formiz autoForm connect={form}>
          <ModalHeader>{isEdit ? 'Update ' : 'Add '} Custom Entry</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
            <FieldInput
              name="name"
              placeholder="Enter Custom Entry"
              required={'Custom Entry required'}
