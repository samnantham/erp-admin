...
const FOBList = () => {
  ...
  const handleSoftDelete = (item: DataColumn, reason: string) => {
    ...
    handleDelete({
      ...
      permanent: false,
      url: endPoints.soft_delete.fob.replace(':id', String(item.id)),
    });
  };
  ...
  const handleRestore = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.restore.fob.replace(':id', String(item.id)),
    });
  };
  ...
  const handlePermanentDelete = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.delete.fob.replace(':id', String(item.id)),
    });
  };
