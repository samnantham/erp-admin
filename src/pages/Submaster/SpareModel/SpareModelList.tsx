...
const SpareModelList = () => {
  ...
  const {
    ...
    refetch: refreshData,
  } = useSpareModelIndex(queryParams);
  ...
  const handleSoftDelete = (item: DataColumn, reason: string) => {
    ...
    handleDelete({
      ...
      permanent: false,
      url: endPoints.soft_delete.spare_model.replace(':id', String(item.id)),
    });
  };
  ...
  const handleRestore = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.restore.spare_model.replace(':id', String(item.id)),
    });
  };
  ...
  const handlePermanentDelete = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.delete.spare_model.replace(':id', String(item.id)),
    });
  };
