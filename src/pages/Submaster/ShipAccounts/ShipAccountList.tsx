...
const ShipAccountsList = () => {
  ...
  const {
    ...
    refetch: refreshData,
  } = useShipAccountIndex(queryParams);
  ...
  const [activeItem, setActiveItem] = useState<ShipACDataColumn | null>(null);

  const { mutateAsync: deleteOrRestoreItem, isLoading: actionLoaderStatus } =
    useDelete<ApiResp, Vars>({
      invalidate: ['shipAccountIndex'],
      request: (vars) => ({
        url: vars.url,
        method: 'DELETE',
  ...
  const openSoft = (item: ShipACDataColumn) => {
    setActiveItem(item);
    setConfirmMode('soft');
  };
  const openRestore = (item: ShipACDataColumn) => {
    setActiveItem(item);
    setConfirmMode('restore');
  };
  const openPermanent = (item: ShipACDataColumn) => {
    setActiveItem(item);
  ...
  const handleSoftDelete = (item: ShipACDataColumn, reason: string) => {
    setMutatingRowId(item.id);
    handleDelete({
      deleted_reason: reason,
      permanent: false,
      url: endPoints.soft_delete.ship_account.replace(':id', String(item.id)),
    });
  };

  const handleRestore = (item: ShipACDataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.restore.ship_account.replace(':id', String(item.id)),
    });
  };

  const handlePermanentDelete = (item: ShipACDataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.delete.ship_account.replace(':id', String(item.id)),
    });
  };
  ...
  const columns = [
    ...
    columnHelper.display({
      ...
      cell: (info) => {
        const rowItem = info.row.original as ShipACDataColumn;
        const isRowBusy = mutatingRowId === rowItem.id && actionLoaderStatus;

        return (
          <SubMasterActions<ShipACDataColumn>
            item={rowItem}
            isBusy={isRowBusy}
