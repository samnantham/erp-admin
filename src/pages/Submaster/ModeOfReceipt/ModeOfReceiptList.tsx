...
const ModeOfReceiptList = () => {
  ...
  const {
    ...
    refetch: refreshData,
  } = useModeOfReceiptIndex(queryParams);
  ...
  const handleSoftDelete = (item: DataColumn, reason: string) => {
    setMutatingRowId(item.id);
    handleDelete({
      deleted_reason: reason,
      permanent: false,
      url: endPoints.soft_delete.mode_of_receipt.replace(':id', String(item.id)),
    });
  };

  const handleRestore = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.restore.mode_of_receipt.replace(':id', String(item.id)),
    });
  };

  const handlePermanentDelete = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.delete.mode_of_receipt.replace(':id', String(item.id)),
    });
  };
  ...
  const columns = [
    ...
    columnHelper.display({
      ...
      cell: (info) => {
        const rowItem = info.row.original as DataColumn;
        const isRowBusy = mutatingRowId === rowItem.id && actionLoaderStatus;

        return (
          <SubMasterActions<DataColumn>
            item={rowItem}
            isBusy={isRowBusy}
