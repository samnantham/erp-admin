...
import { useCreateShipVia, useUpdateShipVia} from '@/services/submaster/ship-via/services';
...
const ModalForm = ({
  ...
  const createItem = useCreateShipVia({
    onSuccess: () => {
      onClose();
      toastSuccess({
        title: 'Ship Via created',
      });
      queryClient.invalidateQueries(['shipViaIndex']);
    },
    onError: (error) => {
      toastError({
        title: 'Error creating Ship Via',
        description: error.response?.data.message,
      });
    },
  });
  
  const updateItem = useUpdateShipVia({
    onSuccess: () => {
      onClose();
      toastSuccess({
    ...
    <Modal initialFocusRef={initialRef} isOpen={isOpen} onClose={onClose} closeOnOverlayClick={false}  closeOnEsc={false}>
      ...
      <ModalContent>
        <Formiz autoForm connect={form}>
          <ModalHeader>{isEdit ? 'Update ' : 'Add '} Ship Via</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
            <FieldInput 
              name="name" 
              placeholder="Enter Ship Via" 
              required={'Ship Via required'}
              type="all-capital"
