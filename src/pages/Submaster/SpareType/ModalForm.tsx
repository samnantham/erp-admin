...
import { useCreateSpareType, useUpdateSpareType } from '@/services/submaster/sparetype/services';
...
const ModalForm = ({
  ...
  const createItem = useCreateSpareType({
    onSuccess: () => {
      onClose();
      toastSuccess({
    ...
    onError: (error) => {
      toastError({
        title: 'Error creating Spare Type',
        description: error.response?.data.message,
      });
    },
  });
  
  const updateItem = useUpdateSpareType({
    onSuccess: () => {
      onClose();
      toastSuccess({
        title: 'Spare Type updated',
      });
      queryClient.invalidateQueries(['spareTypeIndex']);
    },
    onError: (error) => {
    ...
    <Modal initialFocusRef={initialRef} isOpen={isOpen} onClose={onClose} closeOnOverlayClick={false}  closeOnEsc={false}>
      ...
      <ModalContent>
        <Formiz autoForm connect={form}>
          <ModalHeader>{isEdit ? 'Update ' : 'Add '} Spare Type</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
            <FieldInput 
              name="name" 
              placeholder="Enter Spare Type" 
              required={'Spare Type required'}
              type="all-capital"
