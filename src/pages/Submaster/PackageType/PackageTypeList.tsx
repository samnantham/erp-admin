...
const PackageTypeList = () => {
  ...
  const {
    ...
    refetch: refreshData,
  } = usePackageTypeIndex(queryParams);
  ...
  const handleSoftDelete = (item: DataColumn, reason: string) => {
    ...
    handleDelete({
      ...
      permanent: false,
      url: endPoints.soft_delete.package_type.replace(
        ':id',
        String(item.id)
      ),
    });
  ...
  const handleRestore = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.restore.package_type.replace(':id', String(item.id)),
    });
  };
  ...
  const handlePermanentDelete = (item: DataColumn) => {
    setMutatingRowId(item.id);
    handleDelete({
      url: endPoints.delete.package_type.replace(':id', String(item.id)),
    });
  };
