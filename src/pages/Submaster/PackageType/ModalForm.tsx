...
import { useCreatePackageType, useUpdatePackageType } from '@/services/submaster/packagetype/services';
...
const ModalForm = ({
  ...
  const createItem = useCreatePackageType({
    onSuccess: () => {
      onClose();
      toastSuccess({
    ...
    onError: (error) => {
      toastError({
        title: 'Error creating Package Type',
        description: error.response?.data.message,
      });
    },
  });
  
  const updateItem = useUpdatePackageType({
    onSuccess: () => {
      onClose();
      toastSuccess({
    ...
    onError: (error) => {
      toastError({
        title: 'Error updating Package Type',
        description: error.response?.data.message,
      });
    },
  });
        ...
        <Formiz autoForm connect={form}>
          ...
          <ModalBody pb={6}>
            <FieldInput
              ...
              required={'Package Type required'}
