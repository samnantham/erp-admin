...
} from '@/services/submaster/bin-location/services';
...
const ModalForm = ({
  ...
  const createItem = useCreateBinLocation({
    onSuccess: () => {
      onClose();
      toastSuccess({
        title: 'Bin Location created',
      });
      queryClient.invalidateQueries(['binLocationIndex']);
    },
    onError: (error) => {
      toastError({
        title: 'Error creating Bin Location',
        description: error.response?.data.message,
      });
    },
  });

  const updateItem = useUpdateBinLocation({
    onSuccess: () => {
      onClose();
      toastSuccess({
    ...
    <Modal initialFocusRef={initialRef} isOpen={isOpen} onClose={onClose} closeOnOverlayClick={false}  closeOnEsc={false}>
      ...
      <ModalContent>
        <Formiz autoForm connect={form}>
          <ModalHeader>{isEdit ? 'Update ' : 'Add '} Bin Location</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
            <FieldInput
              name="name"
              placeholder="Enter Bin Location Name"
              required={'Bin Location Name required'}
              type="all-capital"
