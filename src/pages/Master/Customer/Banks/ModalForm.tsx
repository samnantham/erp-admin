...
export function BankModal({
  ...
  const form = useForm({
    onValidSubmit: (values) => {
      // Build payload locally — no API call
      const payload = Object.fromEntries(
        KEYS.map((k) => [k, values[k] ?? ''])
      );
      // Preserve existing id on edit so parent can match the record
      if (isEdit && existValues?.id !== undefined) {
        (payload as any).id = existValues.id;
      }
