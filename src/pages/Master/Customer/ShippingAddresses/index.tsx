...
import { ShippingAddressModal } from '@/pages/Master/Customer/ShippingAddresses/ModalForm';
import {
...
type CustomerShippingAddressesProps = {
    fields: ShippingAddressField[];
    onAdd: (data: ShippingAddressField) => void;
    onRemove: (index: number) => void;
    onEdit: (index: number, data: ShippingAddressField) => void;
    customerInfo?: any;
};

// Fields expected in the CSV (customer_code stripped on upload)
const CSV_FIELDS: string[] = import.meta.env.VITE_CUSTOMERS_SHIPPING_ADDRESS_BULK_UPLOAD_CSV_FIELDS
    ? JSON.parse(import.meta.env.VITE_CUSTOMERS_SHIPPING_ADDRESS_BULK_UPLOAD_CSV_FIELDS)
    : [];

export const CustomerShippingAddresses = ({
    fields,
    onAdd,
    onRemove,
    onEdit,
    customerInfo,
}: CustomerShippingAddressesProps) => {
    const [isOpen, setIsOpen] = useState(false);
    const [selected, setSelected] = useState<ShippingAddressField | null>(null);
    ...
    const openModal = (item: ShippingAddressField | null, editStatus: boolean, editIndex?: number) => {
        setSelected(item);
    ...
    const closeModal = (status: boolean, editStatus: boolean, data: ShippingAddressField | null) => {
        setIsOpen(false);
        setSelected(null);
        setIsEdit(false);
        ...
        if (status && data) {
            if (editStatus && indexToEdit !== null) {
                ...
                const payload: ShippingAddressField = { ...data };
    ...
    const handleCSVConfirm = async () => {
        ...
        const mappedRows: ShippingAddressField[] = parsedRows.map((row: any) => {
        ...
        const uniqueRows: ShippingAddressField[] = [];
        ...
        toastSuccess({ title: `${uniqueRows.length} shipping address(es) added from CSV.` });
    ...
    return (
        <Box mt={1}>
            <Flex justify="space-between" align="center" mb={2}>
                ...
                <HStack spacing={2}>
                    <Button
                        ...
                        colorScheme="blue"
                        size="sm"
                        onClick={() => handleDownload(import.meta.env.VITE_CUSTOMERS_SHIPPING_ADDRESSES_CSV)}
                    >
                        Download Sample
                    </Button>
                    ...
                    <Tooltip label="Remove customer_code from file before upload" hasArrow placement="top">
                        <input
                            type="file"
                            id="shipping-address-file-upload"
                            accept=".csv"
                            style={{ display: 'none' }}
                        ...
                        <Button
                            ...
                            variant="solid"
                            as="label"
                            htmlFor="shipping-address-file-upload"
                            size="sm"
                        >
                            Upload File
                        </Button>
            ...
            <ConfirmationPopup
                ...
                headerText={isCSVConfirm ? 'Upload CSV Shipping Addresses' : 'Remove Shipping Address'}
                bodyText={
                    isCSVConfirm
                        ? 'Are you sure you want to import shipping addresses from this CSV file?'
